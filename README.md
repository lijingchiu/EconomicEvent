...
| ISM PMI | <https://www.ismworld.org/supply-management-news-and-reports/reports/rob-report-calendar/> | HTML release calendar + official monthly report pages | Manufacturing/Services 依官方月表，10:00 a.m. Eastern；發布後回填 PMI Actual/Prior | 已線上確認 2026 dates、holiday adjustment 與月報欄位 |
...
發布後的值欄位會優先讀官方資料：BLS 使用官方 API（包含 CPI、PPI、Employment Situation、JOLTS），EIA 使用 WPSR Table 1 CSV 與 WNGSR 官方 JSON，Census 使用 Retail Sales、New Residential Construction 與 Durable Goods 官方發布頁，ISM 使用官方月報表格，University of Michigan 使用官方結果頁；當 release date 對上排程日期時，事件會帶入官方 Actual 與 Prior。沒有單一數值的 Federal Reserve minutes、press conference、speech、testimony 與 Beige Book 不會硬填。
...
目前 provider 優先使用官方 schedule/JSON；BLS、EIA、Census、ISM 與 University of Michigan 的值回填不需要額外 API key。BEA 目前只同步官方發布排程，尚未自動回填數值；若未來接入需金鑰的 API，請以 Worker secret 設定，不要把 key 放到 `vars` 或 source code。
...
- `* * * * *`：每分鐘檢查 D1 due delivery，並重試發布後 48 小時內仍缺少的 Actual/Prior。
- `7 */6 * * *`：每 6 小時同步未來事件。
- `23 18 * * *`：每日完整同步。
...
- 發布前提醒只接受 `scheduled_for_utc <= now` 且大於 now 減 3 分鐘；發布結果保留 60 分鐘補送窗口。過期 delivery 會標記 `expired`，不會在事件公布後誤發「即將公布」通知。

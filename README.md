...
本系統主要做「官方公布時間提醒」。BLS 的 CPI、核心 CPI、PPI、非農、失業率，以及 University of Michigan Consumer Sentiment 會在公布後自動同步官方 Actual / Prior；Forecast 是市場共識而非官方統計，目前不自行猜測，沒有授權來源時顯示 `—`。系統不提供即時行情、新聞分析或做多/做空方向建議。事件時間可能被官方臨時調整，請以來源頁面為準。
...
數值同步另外使用 BLS Public Data API 與 University of Michigan 官方首頁。BLS 月增、年增與非農變動值由官方時間序列計算；每次寫入前都會核對資料期別，官方端尚未發布當期資料時不會把上一期數值誤寫進新事件。公布後會在 0、1、2、3、5、8、12、20、30 分鐘重試，每 6 小時完整同步也會補抓過去 24 小時遺漏值。

開發時已用官方 BLS API 與 University of Michigan 首頁確認現行資料格式；repository 只保留最小化測試資料，不保存第三方完整回應，也不執行會發送正式 Discord 訊息的測試。部署後可透過管理頁的「更新數值」或 `/admin/refresh-values` 立即驗證。
...
排程 provider 優先使用不需要 API key 的官方 schedule/JSON。BLS 數值同步可直接使用公開 API；建議另以 `npx wrangler secret put BLS_API_KEY` 設定免費 registration key 以提高額度。`BEA_API_KEY` 或 `EIA_API_KEY` 也必須以 secret 設定，不要放到 `vars` 或 source code。
...
- `* * * * *`：每分鐘檢查 D1 due delivery，並在公布重試窗口同步官方數值。
- `7 */6 * * *`：每 6 小時同步未來事件，並補抓過去 24 小時遺漏數值。
- `23 18 * * *`：每日完整同步並補抓數值。
...
# 立即補抓過去 24 小時尚未填入的官方數值
curl -X POST https://YOUR_WORKER.workers.dev/admin/refresh-values \
  -H "Authorization: Bearer YOUR_ADMIN_TOKEN"
...
- Forecast 屬於市場共識，官方統計來源通常不提供。接入前需要使用者授權的商業資料 API；系統不會用新聞摘要或模型猜值。
- ISM 報告包含再散布限制，因此目前只同步官方發布排程，不自動建立 ISM 數值資料流。EIA 官方 API 也可能在發布後延遲，未列入公布當下的自動數值來源。
- Cloudflare Cron 與官方網站更新都不是交易所級即時串流；正常情況會在公布後約一分鐘內開始嘗試，但不保證秒級到達。
- 系統不宣稱事件對 NQ、黃金、原油、美元或殖利率的方向。`affectedMarkets` 僅是可能受關注的市場分類，不是交易建議。

## 測試涵蓋

測試包含七個 provider adapter fixture、BLS Actual/Prior 計算、舊期別拒絕寫入、Michigan preliminary/final 期別檢查、JSON/HTML/ICS/XML parser、空資料與 HTTP 失敗、Content-Type、timezone/DST、名稱 aliases 與 false-positive、deterministic ID、Discord 200/204/429/500/400、allowed mentions、admin Bearer token，以及不把 webhook URL 放進 payload/log 的安全邊界。正式 smoke test 請在你自己的 Cloudflare/Discord 環境手動執行 `/admin/test-discord`。

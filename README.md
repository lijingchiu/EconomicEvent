...
發布後的值欄位會優先讀官方資料：BLS 使用官方 API（包含 CPI、PPI、Employment Situation、JOLTS），EIA 使用 WPSR Table 1 CSV 與 WNGSR 官方 JSON，ISM 使用官方月報表格，University of Michigan 使用官方結果頁；當 release date 對上排程日期時，事件會帶入官方 Actual 與 Prior。沒有單一數值的 Federal Reserve minutes、press conference、speech、testimony 與 Beige Book 不會硬填。
...
目前 provider 優先使用官方 schedule/JSON；BLS、EIA、ISM 與 University of Michigan 的值回填不需要額外 API key。BEA 與 Census 的官方數值 API 需要各自的 key，尚未在 Worker 中啟用自動回填；若未來接入，請以 secret 設定 `BEA_API_KEY` 或 `CENSUS_API_KEY`，不要把 key 放到 `vars` 或 source code。

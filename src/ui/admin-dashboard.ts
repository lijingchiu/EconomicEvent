...
<body>
  ...
  <script>
    (function(){
      ...
      var onRefreshValues=async function(){setBusyAction('refresh-values',true);try{var result=await api('/admin/refresh-values',{method:'POST'}),count=(result.result&&result.result.updatedEvents)||0;showToast(count?'已更新 '+count+' 筆官方數值':'官方尚未發布新的數值');await load()}catch(error){showToast(error.message,true)}finally{setBusyAction('refresh-values',false)}};

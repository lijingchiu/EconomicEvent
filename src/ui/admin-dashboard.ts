...
<body>
      ...
      <div class="content-grid">
        ...
        <div class="side-column">
          ...
          <section class="editorial-panel" id="settings-section">
            <div class="panel-head"><div><h2 class="panel-title">通知設定</h2><p class="panel-sub">提醒時間於下一次事件同步套用；其他設定立即生效。</p></div><span class="panel-code">§ 04</span></div>
          ...
          <section class="editorial-panel" id="providers-section">
            ...
            <div class="notice">BLS、EIA、Census、ISM 與 Michigan 的支援事件會在發布後更新官方 Actual / Prior；Forecast 屬市場共識，官方通常不提供。演說、聽證等非量化事件顯示為不適用。</div>
          </section>
        </div>
      </div>
  ...
  <script>
    (function(){
      ...
      var eventMetric=function(event,value){return qualitative(event)?tr('不適用'):metric(value,event.valueUnit)};
      ...
      var renderProviderFilter=function(events){var current=state.provider,names=[];events.forEach(function(event){if(names.indexOf(event.provider)<0)names.push(event.provider)});$('filter-provider').innerHTML='<option value="all">'+esc(tr('所有來源'))+'</option>'+names.sort().map(function(name){return'<option value="'+esc(name)+'">'+esc(providerLabel[name]||name)+'</option>'}).join('');$('filter-provider').value=names.indexOf(current)>=0?current:'all'};
      ...
      $('login-form').addEventListener('submit',function(event){event.preventDefault();var value=$('token').value.trim();if(!value){$('login-error').textContent=tr('請輸入 ADMIN_TOKEN');return}sessionStorage.setItem(tokenKey,value);$('login-error').textContent=tr('驗證中…');$('login-submit').disabled=true;load().catch(function(error){sessionStorage.removeItem(tokenKey);$('login-error').textContent=error.message}).finally(function(){ $('login-submit').disabled=false;})});
      ...
      $('test-discord').addEventListener('click',async function(){setBusyAction('test-discord',true);try{await api('/admin/test-discord',{method:'POST'});showToast(tr('Discord 測試訊息已發送'))}catch(error){showToast(error.message,true)}finally{setBusyAction('test-discord',false)}});
      $('save-settings').addEventListener('click',async function(){var settings=currentSettings();if(!settings.reminderMinutes.length){showToast(tr('至少選擇一個提醒時間'),true);return}setBusyAction('save-settings',true);try{var result=await api('/admin/settings',{method:'PUT',body:JSON.stringify(settings)});state.data.settings=result.settings;renderSettings(result.settings);showToast(tr('設定已保存'))}catch(error){showToast(error.message,true)}finally{setBusyAction('save-settings',false)}});
